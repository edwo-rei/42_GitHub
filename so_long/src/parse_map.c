...
/*   Updated: 2025/11/15 13:46:01 by edwo-rei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	measure_map(t_map *map)
{
	...
	close(fd);
	if (flag == 1)
		error_msg("Map is not rectangular or has extra spaces");
	if (map->width == 0)
...
void	fill_grid(t_map *map)
{
	...
	if (!map->grid)
		error_msg("Failed grid malloc");
	map->check = malloc(sizeof (char *) * map->height);
	if (!map->check)
	{
		free(map->grid);
		error_msg("Failed check malloc");
	}
...
void	parse_map(t_map *map)
{
	...
	validate_path(map, map->p1.y, map->p1.x);
