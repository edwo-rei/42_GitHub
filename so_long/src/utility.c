...
/*   Updated: 2025/11/15 14:34:01 by edwo-rei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//init game w/ NULL for ptrs to ensure functioning of clean_up, set other
//calues to 0
...
//type used for the result returned by the sizeof operator. size_t is chosen so
...
int	check_map_name(char *name)
{
	...
	i = ft_strlen(name) - 1;
	if (name[i - 4] && name[i - 4] != '/' && name[i - 3] == '.' 
		&& name[i - 2] == 'b' && name [i - 1] == 'e' && name[i] == 'r')
		return (0);
	else
		return (1);
}
